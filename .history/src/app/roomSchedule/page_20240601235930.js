...
export default async function RoomSchedule() {
  ...
  const lichphongDetails = lichphongResult.map(lp => filter(lp.Phong_id === phongResult.id))
  console.log(lichphongDetails);
            ...
            >
              ...
              <div className='absolute top-[90px] left-0 w-full h-[calc(100%-90px)]'>
                <div className='relative size-full' >
                  {
                    lichphongDetails && lichphongDetails.map((lp, index) => {

                      const top = (new Date(lp.GioBatDau).getHours() - 7) * 60 + new Date(lp.GioBatDau).getMinutes();
                      return (
